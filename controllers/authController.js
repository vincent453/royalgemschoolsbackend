  ...
  try {
    ...
    if (regNumber && pin) {
      ...
      const pinDoc = await Pin.findOne({
        pin: pin.trim(),
        $or: [{ usedBy: student._id }, { usedBy: null, isUsed: false }],
      });
      ...
      if (pinDoc.expiresAt && new Date() > pinDoc.expiresAt) {
        return res.status(401).json({ message: "This PIN has expired. Please contact the school office." });
      }
      if (!pinDoc.usedBy) {
      ...
      return res.json({
        ...
        student: {
          ...
          regNumber: student.regNumber, classLevel: student.classLevel,
          session: student.session, gender: student.gender,
          profilePhoto: student.profilePhoto,
          parentFirstName: student.parentFirstName, parentLastName: student.parentLastName,
          parentPhone: student.parentPhone, parentEmail: student.parentEmail,
        },
      });
    }

    // ── BRANCH B: email + password → staff / super admin ───────
    if (email && password) {
      // 1. Super Admin (Admin collection)
      ...
      if (admin) {
        ...
        return res.json({
          ...
          user: { _id: admin._id, name: admin.name, email: admin.email, role: "super_admin" },
        });
      }

      // 2. Staff (User collection)
...
export const registerUser = async (req, res) => {
  try {
    ...
    if ((role === "student" || role === "parent") && !studentId) {
      return res.status(400).json({ message: `Student ID is required for ${role}s` });
    }
    if (studentId) {
    ...
    const user = await User.create({ name, email, password, role, student: studentId || undefined, phoneNumber });
    const token = jwt.sign({ id: user._id, role: user.role }, process.env.JWT_SECRET, { expiresIn: "7d" });

    res.status(201).json({
      message: "User registered successfully", token,
      user: { _id: user._id, name: user.name, email: user.email, role: user.role, student: user.student },
    });
  } catch (error) {
...
// ─────────────────────────────────────────────────────────────
// ADMIN ENDPOINTS (kept for backward compatibility)
// ─────────────────────────────────────────────────────────────
...
export const logoutAdmin = async (req, res) => {
  res.json({ message: "Logout successful", instructions: "Please delete the token from your client storage" });
};
...
export const getAdminProfile = async (req, res) => {
  try {
    ...
    res.json({ _id: admin._id, name: admin.name, email: admin.email, createdAt: admin.createdAt, updatedAt: admin.updatedAt });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
};
...
export const updateAdminProfile = async (req, res) => {
  try {
    ...
    res.json({ message: "Profile updated successfully", admin: { _id: updatedAdmin._id, name: updatedAdmin.name, email: updatedAdmin.email } });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
};
